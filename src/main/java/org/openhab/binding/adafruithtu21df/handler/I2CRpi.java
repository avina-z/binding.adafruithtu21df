...
public class I2CRpi {
    ...
    public I2CRpi(int i2cAddress) throws IOException {
        //config = new I2CDeviceConfig(DeviceConfig.DEFAULT, i2cAddress, DeviceConfig.DEFAULT, DeviceConfig.DEFAULT);

        config = new I2CDeviceConfig.Builder()
                .setControllerNumber(DeviceConfig.DEFAULT)
                .setAddress(i2cAddress, DeviceConfig.DEFAULT)
                .setClockFrequency(DeviceConfig.DEFAULT)
                .build();

        device = DeviceManager.open(I2CDevice.class, config);
    }
